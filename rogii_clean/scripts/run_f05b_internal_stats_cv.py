"""运行 B00 12 列加 F05b 10 列的固定单模 LightGBM CV。"""
...
EXPERIMENT_ID = "F05b_internal_stats_v1"
F05B_COLUMNS = list(F05B_FEATURE_COLUMNS)
DEFAULT_CONFIG = CLEAN_ROOT / "configs" / "f05b_internal_stats_v1.json"
SUPPORTED_MODES = ["smoke", "fold0", "fold1", "fold2", "fold3", "fold4"]
EXPECTED_CACHE_COLUMNS = ["well_id", "row_index", *F05B_COLUMNS]


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行 F05b 归一化 PF 内部统计 CV")
    parser.add_argument("--mode", required=True, choices=SUPPORTED_MODES)
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    return parser.parse_args(argv)


def require_internal_stats_cache(cache_path: Path, metadata_path: Path) -> None:
    """CV 只读取完整缓存，不在训练时偷偷重建。"""
...
def validate_exact_key_alignment(
    ...
    """F05b 缓存必须与 B00 的 well_id、row_index 逐行同序。"""

    if len(base_rows) != len(internal_stats_rows):
        raise ValueError("F05b 缓存与 B00 键顺序不一致：行数不同")
    base_wells = base_rows["well_id"].astype(str).to_numpy()
    stats_wells = internal_stats_rows["well_id"].astype(str).to_numpy()
    base_indices = pd.to_numeric(base_rows["row_index"], errors="raise").to_numpy(
        dtype=np.int64
    )
    stats_indices = pd.to_numeric(
        internal_stats_rows["row_index"], errors="raise"
    ).to_numpy(dtype=np.int64)
    if not np.array_equal(base_wells, stats_wells) or not np.array_equal(
        base_indices,
        stats_indices,
    ):
        raise ValueError("F05b 缓存与 B00 键顺序不一致")
...
def read_cache_provenance(cache_path: Path, metadata_path: Path) -> dict:
    require_internal_stats_cache(cache_path, metadata_path)
    return {
...
def _validate_config(config: dict) -> list[str]:
    model_features = [*FEATURE_COLUMNS, *F05B_COLUMNS]
    if config.get("experiment_id") != EXPERIMENT_ID:
        raise ValueError("配置 experiment_id 与 F05b runner 不一致")
    if config.get("feature_columns") != model_features:
        raise ValueError("正式特征必须是 B00 12 列加 F05b 10 列")
    if config.get("internal_stat_feature_columns") != F05B_COLUMNS:
        raise ValueError("配置中的 F05b 特征列或顺序已变化")
    if config.get("feature_count") != 22:
        raise ValueError("F05b 正式模型必须恰好使用 22 个特征")
    if config.get("model_config") != "configs/lgbm_feature_baseline_v1.json":
        raise ValueError("F05b 必须使用冻结的单模 LightGBM 配置")
    if int(config.get("n_estimators", -1)) != 1734:
        raise ValueError("F05b 必须固定使用 1734 棵树")
    if int(config.get("model_seed", -1)) != 29:
        raise ValueError("F05b 必须固定使用 LightGBM seed 29")
    if config.get("early_stopping") is not False:
        raise ValueError("F05b 禁止 early stopping")
    if config.get("target") != "target_delta":
        raise ValueError("F05b target 必须保持为 TVT 减最后可见 TVT")
    if config.get("cache_rebuild_allowed") is not False:
        raise ValueError("F05b CV 禁止自行重建缓存")
    if config.get("supported_modes") != SUPPORTED_MODES:
        raise ValueError("配置支持的运行模式与 runner 不一致")
    return model_features


def _validate_cache_metadata(metadata: dict, expected_wells: int, expected_rows: int) -> None:
    if metadata.get("experiment_id") != "F05b_internal_stats_cache_v1":
        raise ValueError("F05b 缓存 experiment_id 不正确")
    if metadata.get("completed") is not True:
        raise ValueError("F05b 缓存尚未完整生成")
    if int(metadata.get("wells", -1)) != expected_wells:
        raise ValueError("F05b 缓存井数不正确")
    if int(metadata.get("rows", -1)) != expected_rows:
        raise ValueError("F05b 缓存行数不正确")
    if metadata.get("keys_unique") is not True:
        raise ValueError("F05b 缓存复合键不唯一")
    if metadata.get("registry_order_preserved") is not True:
        raise ValueError("F05b 缓存未保持固定 fold 注册表顺序")
    if metadata.get("feature_columns") != F05B_COLUMNS:
        raise ValueError("F05b 缓存特征列或顺序不正确")
    if int(metadata.get("feature_count", -1)) != 10:
        raise ValueError("F05b 缓存必须恰好包含 10 个特征")
    if int(metadata.get("seed", -1)) != 42:
        raise ValueError("F05b 缓存不是固定 seed=42")
    if not str(metadata.get("cache_sha256", "")):
        raise ValueError("F05b 缓存元数据缺少 SHA-256")
    if not str(metadata.get("cache_fingerprint", "")):
        raise ValueError("F05b 缓存元数据缺少生成指纹")


def load_aligned_feature_table(
    base_cache_path: Path,
    internal_stats_cache_path: Path,
    expected_rows: int,
) -> pd.DataFrame:
    if not base_cache_path.is_file():
        raise FileNotFoundError(f"缺少 B00 特征缓存：{base_cache_path.resolve()}")
    stats_rows = pd.read_parquet(internal_stats_cache_path)
    if list(stats_rows.columns) != EXPECTED_CACHE_COLUMNS:
        raise ValueError("F05b 缓存列或顺序不正确")
    if len(stats_rows) != expected_rows:
        raise ValueError("F05b 缓存行数与固定评价行不一致")
    if stats_rows.duplicated(["well_id", "row_index"]).any():
        raise ValueError("F05b 缓存含重复键")
    values = stats_rows[F05B_COLUMNS].to_numpy(dtype=np.float32)
    if not np.isfinite(values).all():
        raise ValueError("F05b 缓存含 NaN 或 Inf")

    base_rows = pd.read_parquet(base_cache_path)
    if len(base_rows) != expected_rows:
        raise ValueError("B00 特征缓存行数与固定评价行不一致")
    validate_exact_key_alignment(base_rows, stats_rows)
    return pd.concat(
        [
            base_rows.reset_index(drop=True),
            stats_rows[F05B_COLUMNS].reset_index(drop=True),
        ],
        axis=1,
    )
...
def _run_smoke(feature_table: pd.DataFrame) -> None:
    ...
    if len(smoke_wells) != 3 or not np.isfinite(values).all():
        raise ValueError("F05b smoke 检查失败")
    print(
...
def main(argv: list[str] | None = None) -> None:
    ...
    if int(model_params.get("n_estimators", -1)) != 1734:
        raise ValueError("固定 LightGBM 必须使用 1734 棵树")
    if int(model_params.get("random_state", -1)) != 29:
        raise ValueError("固定 LightGBM 必须使用 seed 29")
